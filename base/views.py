...
from .forms import SubmssionForm, CustomUserCreationForm
...
def project_submission(request, pk):
    ...
    form = SubmssionForm()
    if request.method =='POST':
        form = SubmssionForm(request.POST)
        if form.is_valid():
...
def update_submission(request, pk):
    ...
    form = SubmssionForm(instance=submission)
    if request.method == 'POST':
        form = SubmssionForm(request.POST,instance=submission)
        if form.is_valid():
            form.save()
            return redirect('account')
